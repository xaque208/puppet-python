...
EOS
  ...
  describe 'python_version' do
    context 'returns Python version when `python` present' do
      it do
        allow(Facter::Util::Resolution).to receive(:which).with('python') { true }
        allow(Facter::Util::Resolution).to receive(:exec).with('python -V 2>&1') { python2_version_output }
  ...
  describe 'python2_version' do
    ...
    context 'returns Python 2 version when `python` is Python 3 and `python2` is present' do
      it do
        allow(Facter::Util::Resolution).to receive(:which).with('python') { true }
        allow(Facter::Util::Resolution).to receive(:exec).with('python -V 2>&1') { python3_version_output }
        expect(Facter::Util::Resolution).to receive(:which).with('python2') { true }
        allow(Facter::Util::Resolution).to receive(:exec).with('python2 -V 2>&1') { python2_version_output }
        Facter.value(:python2_version).should == '2.7.9'
      end
    end
    ...
    context 'returns nil when `python` is Python 3 and `python2` is absent' do
      it do
        allow(Facter::Util::Resolution).to receive(:which).with('python') { true }
        allow(Facter::Util::Resolution).to receive(:exec).with('python -V 2>&1') { python3_version_output }
        allow(Facter::Util::Resolution).to receive(:which).with('python2') { false }
    ...
    context 'returns nil when `python2` and `python` are absent' do
      it do
        allow(Facter::Util::Resolution).to receive(:which).with('python') { false }
        allow(Facter::Util::Resolution).to receive(:which).with('python2') { false }
        Facter.value(:python2_version).should.nil?
      end
    end
  ...
  describe 'python3_version' do
    context 'returns Python 3 version when `python3` present' do
      it do
        allow(Facter::Util::Resolution).to receive(:which).with('python3') { true }
        allow(Facter::Util::Resolution).to receive(:exec).with('python3 -V 2>&1') { python3_version_output }
    ...
    context 'returns nil when `python3` not present' do
      it do
        allow(Facter::Util::Resolution).to receive(:which).with('python3') { false }
